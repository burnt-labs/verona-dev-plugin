...
metadata:
  ...
  recommends:
    - verona-dev
  compatibility: Requires verona-toolkit CLI and browser for OAuth2 flow
---
...
This skill wraps the `verona-toolkit` CLI tool to provide Agent-friendly OAuth2 authentication capabilities:
...
**Common Error Codes:**
- `CLI_NOT_FOUND` - verona-toolkit CLI not found in PATH
...
| CLI not found | Install via `verona-toolkit-init` skill |
...
```bash
skills/verona-dev/scripts/validate-params.sh verona-oauth2 login '{}'
```

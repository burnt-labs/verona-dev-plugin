...
description: |
  ...
  Use AFTER verona-oauth2 skill - authentication is required for claim operations.
metadata:
  author: burnt-labs
  version: "1.2.2"
  requires:
  ...
  recommends:
    - verona-dev
compatibility: Requires verona-toolkit CLI and OAuth2 authentication (for claim operations)
---
...
| `EFAUCET003` | Not authenticated | Run `verona-toolkit auth login` |
...
verona-toolkit send <recipient> 500000uxion
```
...
Or use the flag:
```bash
verona-toolkit faucet claim --network testnet
```

## Related Skills

- **verona-oauth2** - Authentication (use before this skill for claim)
- **verona-toolkit-init** - CLI installation (use if CLI not found)
- **verona-treasury** - Treasury management after obtaining tokens
...
```bash
skills/verona-dev/scripts/validate-params.sh verona-faucet claim '{"receiver": "xion1abc..."}'
```

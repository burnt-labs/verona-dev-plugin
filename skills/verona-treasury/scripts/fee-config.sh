#!/bin/bash
#
# verona-treasury/fee-config.sh - Configure Fee Grants for Treasury
#
# This script configures Fee Grants for a Treasury using the verona-toolkit CLI.
#
# Usage:
...
if ! command -v verona-toolkit &> /dev/null; then
    output_json '{
        "success": false,
        "error": "verona-toolkit CLI not found in PATH",
...
CMD=(verona-toolkit --no-interactive --network "$NETWORK" treasury fee-config)

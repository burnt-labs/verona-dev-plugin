#!/bin/bash
#
# verona-treasury/create.sh - Create a new Treasury contract
#
...
check_cli() {
    if ! command -v verona-toolkit &> /dev/null; then
        output_json '{
            ...
            "suggestion": "Install verona-toolkit CLI or ensure it is in your PATH"
        }'
        exit 1
    fi
}
...
CMD=(verona-toolkit --no-interactive treasury create --output json)

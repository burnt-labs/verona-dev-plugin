#!/bin/bash
#
# verona-treasury/query.sh - Query Treasury details
#
...
CMD=(verona-toolkit --no-interactive treasury query "$ADDRESS" --output json)
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    if echo "$RESULT" | grep -qi "not authenticated\|no credentials\|not logged in"; then
        handle_error "Not authenticated. Please use 'verona-oauth2/login.sh' first." "NOT_AUTHENTICATED"

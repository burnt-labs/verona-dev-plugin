...
MetaAccount, gasless, 无 gas, xion toolkit, xion 开发, xion 开发入门, xion setup, install xion, xion 安装, xion sdk, verona-toolkit upgrade, update verona-toolkit, xion 升级, xion 版本更新.
...
1. **Authenticate** → Use `verona-oauth2` skill
2. **Manage Treasuries** → Use `verona-treasury` skill
3. **Claim Testnet Tokens** → Use `verona-faucet` skill
...
```
Follow this guide https://raw.githubusercontent.com/burnt-labs/verona-agent-toolkit/main/INSTALL-FOR-AGENTS.md to install and configure the Verona Agent Toolkit.
```

## Dependency Graph

```
verona-dev (entry point - routes to correct skill)
    │
    ├── verona-toolkit-init (this skill - guidance only)
    │       │
    │       ├── verona-oauth2 (authentication)
    │       │       │
    │       │       └── verona-treasury (gasless operations)
    │       │

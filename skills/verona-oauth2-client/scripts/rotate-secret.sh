#!/bin/bash
#
# verona-oauth2-client/rotate-secret.sh - Rotate OAuth2 client secret
#
...
if ! command -v verona-toolkit &> /dev/null; then
    handle_error "verona-toolkit CLI not found in PATH. Please install verona-agent-toolkit first." "CLI_NOT_FOUND"
fi
...
CMD_STR="${CMD_ARGS[*]}"
log_info "Running: verona-toolkit --no-interactive $CMD_STR"

# Execute verona-toolkit rotate-secret command
# Capture both stdout and stderr, but keep them separate
RESULT=$(verona-toolkit --no-interactive "${CMD_ARGS[@]}" 2>&1)
EXIT_CODE=$?
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    if echo "$RESULT" | grep -qi "not authenticated"; then
        handle_error "Not authenticated. Run 'verona-toolkit auth login --dev-mode' first." "EOAUTHCLIENT008"
